 ...
 *    an unknown backdrop, or a `color-mix()`. Every VERDICT on this page is one:
 *    `.verdict-exposed`, `.verdict-ok`, `.verdict-info` and `.verdict-warn` each
 *    paint `color-mix(in oklab, <their own ink> 8%, transparent)` and then put
 *    that same ink on top of it, so the ratio depends on a composite axe will
 *    not compute. So are both heatmaps: every non-zero cell of the document
 *    co-occurrence matrix and of the attack's input matrix carries an inline
 *    `background-color: color-mix(in oklab, var(--accent) N%, transparent)` with
 *    N scaled up to 35, and `--text` is painted on top. `table.data tbody
 *    tr.is-new` — the row that lands in the server's store as you step the build
 *    — is an 18% accent mix, and the hero aside is a 6% one. axe files all of
 *    those under `incomplete`, which means a violations-only gate measured the
 *    contrast of almost nothing this lab actually decides.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. The only such rule here is
 *    `.cl-hero-sub` at `.85`, and it is measured for real rather than assumed:
 *    the walk composites the fade onto the surface beneath it.
 *
 ...
 *    box is `.scroll-region` — `max-height: 22rem; overflow: auto` — and this
 *    page has TEN of them: the server's store, the complete index across all
 *    fourteen keywords, the query ledger, the overlap table, both heatmaps, the
 *    attack's per-token results, the challenge board, the background-knowledge
 *    table and the comparison table. Several overflow on arrival and all of them
 *    do once the drive has run a round of queries. Content scrolled past them is
 *    not dimmed or partly drawn — it is absent from the frame, and asking what
 *    colour it sits on has no answer. Its rect is still outside every ancestor's
 *    box, so the ancestor walk would find nothing behind it and fall through to
 *    white, inventing a failure. Skip it, and rely on the content that IS in
 *    view, which is measured for real.
 *
 ...
 *    is live here and load-bearing. Exhibit 3's result-set overlap graph is a
 *    real `<svg>`: `<line class="graph-edge">` elements first, then one
 *    `<circle class="graph-node">` per observed token, then a
 *    `<text class="graph-label">` on top of each circle. The circle is filled
 *    `var(--surface)` and the label is `fill: var(--text)`, so the label's
 *    backdrop is a SIBLING, not an ancestor — the `<svg>` itself has no
 *    background at all. An ancestor-only walk would composite the label onto
 *    `.graph-wrap`'s `--surface-2` instead, reporting a ratio the screen does
 *    not have. The stroke-only guard below matters here too: the edges are
 *    `<line>`s, whose computed `fill` is the SVG initial black, and compositing
 *    those as opaque black rectangles would invent failures across the whole
 *    figure.
 *
 ...
 * that gap, and `styles.css` in fact declares no `::before` or `::after` content
 * of its own: the disclosure triangles are the user agent's own markers, and
 * every icon on this page (the verdict glyphs, the badge glyphs, the client and
 * server emoji) is a real `aria-hidden` text node inside an element, which this
 * walk measures directly.
 */
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks the complete encrypted index (one row per
     * posting, across fourteen keywords), two square heatmaps whose side is the
     * number of observed tokens, the query ledger after a forty-query round, and
     * a fourteen-node overlap graph — thousands of sibling elements all
     * re-walking the same ancestors up to `<body>`. Without the caches the pass
     ...
     * that is every `.scroll-region` (`max-height: 22rem; overflow: auto`), of
     * which this page builds ten. `.visually-hidden` is handled separately
     * below.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. BOTH branches are live on this page: the overlap
      // graph's `<circle class="graph-node">` is a genuine filled backdrop for
      // the `<text class="graph-label">` drawn over it, and the same figure's
      // `<line class="graph-edge">` elements are exactly the stroke-only shapes
      // the guard exists for — without it, every edge crossing a node would
      // composite as an opaque black rectangle under the label.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details>` on the page — the intro glossary, the server's complete
      // store, the overlap table, the attack's input matrix and the challenge's
      // background knowledge — all of which ship shut. The gate opens each by
      // clicking its <summary>, which is the route a reader has, rather than
      // setting `.open = true` from script, which is exactly what the gate this
      // replaces did to all of them at once before each of its scans.
      ...
      //
      // `styles.css` here DEFINES `.visually-hidden` with exactly that recipe
      ...
      // 'visually-hidden'})` away from being used, and without it the first such
      // span would be reported at whatever ratio a 1px box happens to composite
      // to, for ink the compositor never lays down. It is deliberately narrow:
     ...
     * the `.verdict-icon` glyph on every verdict block (⚠ 👁 ✓ ℹ ⏱), the leading
     * glyph inside every `.badge`, the 🔑 / 🗄️ / 👁 that open the client and
     * server column titles, and the `.bar-track` of each histogram row. In every
     * case the words that carry the meaning sit immediately beside the glyph, in
     * the same ink on the same surface, so measuring the sibling measures the
     * glyph too — and the bar tracks own no text at all, with the count printed
     * beside them in `.bar-value`.
     *
     * Nothing on this page hides a VALUE. No token, no address, no sealed
     * ciphertext, no result size, no recovery score and no verdict is inside an
     ...
     *
     * This lab builds SVG through `dom.ts`'s `svgEl()`, which sets `text` as a
     * real text node — so the overlap graph's labels are `<text>` and DO render.
     * The guard stays because `svgEl` will happily put a text child on any tag:
     * the same helper that builds a `<text>` builds the `<line>`s and
     * `<circle>`s, and a stray string child on either of those would paint
     * nothing while a walk that measured it reported a 1:1 failure describing
     ...
     *
     * It is not a no-op here: `styles.css` sets `background-color: #0a1413` on
     * `html` ITSELF (and `#f2f8f7` under `[data-theme='light']`), so the root's
     * own paint is opaque and this is the branch that supplies it. The page runs
     * eight exhibits and many screens deep once the drive has populated them, so
     * the alternative — falling through to WHITE for everything the geometry
     * walk finds no ancestor behind — would fabricate failures across most of a
     * near-black page.
     */
